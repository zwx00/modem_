...
const paintMovingSprite = ({ meta, resource, container }) => {
  const surfaceWidth = window.innerWidth;
  const surfaceHeight = window.innerHeight;

  let spriteData = {};
  const sprite = getSprite(resource, meta);
      ...
      .then(() => {
        return new Promise(resolve => {
          new PIXI.Loader()
            .add(object.filename)
            .load((_, resources) =>
              resolve(paintMovingSprite({ meta: object, resource: resources[object.filename]}))
            );
        });
      });
  }).map(
